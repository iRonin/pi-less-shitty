...
describe("collectAutocompleteBasePaths", () => {
	...
	it("handles paths with spaces", () => {
		const spaceDir = join(rootDir, "My Project", "Module Alpha", "Deep Workdir");
		const basePath = join(rootDir, "My Project");
...
describe("fd fuzzy search with spaces and case-insensitive", { skip: !fdPath }, () => {
	...
	it("finds files across base paths with case-insensitive matching", () => {
		...
		writeFile(join(baseDir, "style-guide.md"), "content");
		...
		const found = results.some(r => r.includes("style-guide.md"));
		assert.ok(found, "should find style-guide.md with case-insensitive @style");
	});
	...
	it("fuzzy-matches file with spaces via @styleguide (no spaces in query)", () => {
		const spaceDir = join(rootDir, "My Project With Spaces", "Sub Dir");
		const baseDir = join(rootDir, "My Project With Spaces");
		mkdirSync(spaceDir, { recursive: true });

		writeFile(join(baseDir, "style-guide.md"), "content");
		writeFile(join(baseDir, "Code Rules.pdf"), "content");
		...
		// Simulate what buildFdQueryRegex does: s.*t.*y.*l.*e.*g.*u.*i.*d.*e
		const fuzzyPattern = "styleguide".split("").map((c) => c.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join(".*");
		...
		assert.ok(out.stdout.includes("style-guide.md"), "should fuzzy-match style-guide.md with @styleguide");
	...
	it("handles spaces in paths", () => {
		...
		assert.ok(out.stdout.includes("style-guide.md"), "should find file with spaces in parent dir");
	});
});
