...
export class InteractiveMode {
    ...
    setupAutocomplete(fdPath) {
        ...
        // Setup autocomplete — collect autocompleteBasePaths from cwd + all parent .pi/settings.json
        const collectAutocompleteBasePaths = (cwd) => {
            const paths = [];
            const seen = new Set();
            let dir = cwd;
            while (true) {
        ...
        this.autocompleteProvider = new CombinedAutocompleteProvider([...slashCommands, ...templateCommands, ...extensionCommands, ...skillCommandList], this.sessionManager.getCwd(), fdPath, collectedBasePaths);
    ...
    async shutdown() {
        ...
        await this.runtimeHost.dispose({ skipShutdownEvent: true });
        ...
        this.stop();
        // Emit shutdown event AFTER TUI cleanup so console.error output is visible
        await emitSessionShutdownEvent(this.runtimeHost.session.extensionRunner);
        process.exit(0);
    }
    /**
