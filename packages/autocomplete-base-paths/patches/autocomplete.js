...
export class CombinedAutocompleteProvider {
    ...
    async getFuzzyFileSuggestions(query, options) {
        ...
        try {
            ...
            // If scoped query resolves to a specific directory, search only there
            if (scopedQuery) {
                const entries = await walkDirectoryWithFd(scopedQuery.baseDir, this.fdPath, fdQuery, 100, options.signal);
                if (options.signal.aborted)
                    return [];
                return this.buildFuzzySuggestions(entries, fdQuery, scopedQuery.displayBase, options.isQuotedPrefix);
            }

            // Otherwise search from basePath + all additional base paths, deduplicating by absolute path
            const allBasePaths = [this.basePath, ...this.additionalBasePaths];
            const seenAbsolutePaths = new Set();
            const allEntries = [];

            for (const baseDir of allBasePaths) {
                const entries = await walkDirectoryWithFd(baseDir, this.fdPath, fdQuery, 100, options.signal);
                if (options.signal.aborted)
                    return [];
                for (const entry of entries) {
                    const absolutePath = join(baseDir, toDisplayPath(entry.path));
            ...
            return this.buildFuzzySuggestions(allEntries, fdQuery, undefined, options.isQuotedPrefix);
        }
        catch {
            return [];
        }
    ...
    buildFuzzySuggestions(entries, fdQuery, displayBase, isQuotedPrefix) {
        ...
        for (const { path: entryPath, isDirectory } of topEntries) {
            ...
            const displayPath = displayBase
                ? this.scopedPathForDisplay(displayBase, pathWithoutSlash)
                : pathWithoutSlash;
