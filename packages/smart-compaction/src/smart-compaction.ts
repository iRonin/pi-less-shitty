...
async function generateSmartSummary(
  ...
  const promptText = `You are a session continuation assistant. The agent's conversation was just compacted. The next agent turn will ONLY see this summary — it must contain everything needed to pick up exactly where the agent left off.
...
// fallback-chain.ts so they can be unit-tested without the pi peer-deps.)

// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// Model resolution for compaction summary
// ---------------------------------------------------------------------------

async function resolveCompactionModel(settings: SmartCompactionSettings, ctx: any, modelRegistry: any): Promise<any> {
  // Layer 1: explicit compactionModel in settings
  if (settings.compactionModel) {
    const providerId = settings.scoringProvider;
    console.error(`[smart-compaction] model resolution: looking up ${providerId}:${settings.compactionModel}`);
    const model = modelRegistry.find(providerId, settings.compactionModel);
    if (model) {
      console.error(`[smart-compaction] model resolution: L1 found → ${model.provider}/${model.id} (api=${model.api}, reasoning=${model.reasoning})`);
      return model;
    }
    // Try bare ID against all providers
    const avail = modelRegistry.getAvailable?.() ?? [];
    for (const m of avail) {
      if (m.id === settings.compactionModel || m.id.endsWith("/" + settings.compactionModel)) {
        console.error(`[smart-compaction] model resolution: L1 bare-ID fallback → ${m.provider}/${m.id}`);
        return m;
      }
    }
  ...
  // Layer 3: default provider + model
  const dp = modelRegistry.defaultProvider;
  const dm = modelRegistry.defaultModel;
  if (dp && dm) {
    model = modelRegistry.find(dp, dm);
    if (model) {
      console.error(`[smart-compaction] model resolution: L3 default → ${model.provider}/${model.id}`);
      return model;
    }
  }
  ...
  for (const m of avail) {
    try {
      ...
      if (a?.ok && a?.apiKey) {
        console.error(`[smart-compaction] model resolution: L4 first-available → ${m.provider}/${m.id}`);
        return m;
      }
    } catch {}
  }
...
async function handleSmartCompaction(
  ...
  // Iterate the unfiltered scored array so `i` indexes into flatMsgs correctly.
  // Filtering first would make `idx` an index into the filtered slice — wrong msg.
  const highTokens = scored.reduce(
  ...
  const scoringTag = scoringMethod === "llm"
    ? ` [scoring: ${settings.scoringProvider}:${settings.scoringModel}, summary: ${summaryModelId}]`
    : " [heuristic]";
...
function analyzeMessages(
  ...
  return {
    ...
    stats: {
      ...
      actualDropThreshold: settings.dropThreshold,
      llmUsed: false,
      scoringMethod: "heuristic", // sync path always uses heuristic scorer
      summaryModel: "",
    },
    retryLoops: loops,
    summary,
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    // mid-startup which can swallow stdout writes.
    const flagValue = pi.getFlag("smart-compress") as string | undefined;
    if (flagValue) {
      const writeLine = (s: string) =>
        new Promise<void>((resolve) => process.stdout.write(s + "\n", () => resolve()));

      try {
        const sessionInfo = await findSession(flagValue);
        if (!sessionInfo) {
          await writeLine(`Session not found: "${flagValue}"`);
          process.exitCode = 1;
          ctx.shutdown();
          return;
        }
        ...
        if (flatMsgs.length === 0) {
          await writeLine(`No messages found in session: ${sessionInfo.name}`);
          process.exitCode = 1;
          ctx.shutdown();
          return;
        }
        ...
        await writeLine(`Error analyzing session: ${err.message}`);
        process.exitCode = 1;
      }
      ctx.shutdown();
      return;
