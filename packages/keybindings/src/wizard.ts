...
import { isKeyRelease, parseKey, type KeyId } from "@earendil-works/pi-tui";
...
export async function runAddWizard(
  ...
  // 1. Capture key — wrap in try/finally so captureMode is always reset,
  // even if captureNextKey throws or the user aborts mid-flow.
  setCaptureMode(true);
  let key: string | null;
  try {
    key = await captureNextKey(ctx, () => {
      ctx.ui.notify(
