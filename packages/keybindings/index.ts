...
export default function piKeybindings(pi: ExtensionAPI) {
  // ---------------------------------------------------------------------------
  // State (module-scope inside the closure so it persists across session_start
  // events without re-registering handlers — see handler-leak note below).
  // ---------------------------------------------------------------------------
  ...
  // The most recent session ctx — captured by session_start, read by the
  // singleton input handler below. Prevents the handler from referencing a
  // stale ctx after a session restart.
  ...
  // Registered ONCE at extension load — not inside session_start.
  pi.on("agent_start", () => {
    lastPressAt.clear();
  });
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    // Handler-leak fix: install onTerminalInput once — it reads `activeCtx`
    // so subsequent session_starts only re-point the closure target.
    ...
    ctx.ui.onTerminalInput((data) => {
      ...
      const binding =
        candidates.find((b) => b.double && isDouble && checkWhen(b.when, activeCtx)) ??
        candidates.find((b) => !b.double && checkWhen(b.when, activeCtx));
      ...
      const result = executeActionWithCtx(binding, pi, activeCtx);
