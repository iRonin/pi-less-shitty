 ...
 * - `@earendil-works/pi-coding-agent/dist/core/keybindings.js` — adds keybindings
 */
...
function findKeybindingsJs(override?: string): string | null {
	const res = findPiCodingAgentDistFromCaller(import.meta.url, { override });
	if (!res?.distDir) return null;
	const kbPath = path.join(res.distDir, "core", "keybindings.js");
	return fs.existsSync(kbPath) ? kbPath : null;
}
...
function applyKeybindingsPatch(filePath: string): "patched" | "already" | "failed" {
	...
	// Inject after the deleteNoninvasive entry, regardless of what follows it.
	// Match the entry as a regex to tolerate whitespace/indentation drift
	// and survive upstream additions/reorderings of sibling entries.
	const deleteNoninvasiveEntryRe =
		/("app\.session\.deleteNoninvasive":\s*\{[\s\S]*?"Delete session when query is empty",?\s*\},?)/;

	const match = content.match(deleteNoninvasiveEntryRe);
	if (!match) return "failed";

	const injectNewBindings = `
	...
	content = content.replace(deleteNoninvasiveEntryRe, `$1${injectNewBindings}`);
