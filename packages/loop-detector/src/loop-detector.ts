...
function canonicalKey(toolName: string, args: Record<string, unknown>): string {
  ...
  try {
    normalized = stableStringify(args);
  } catch {
...
export class ToolLoopDetector {
  ...
  private checkPingPong(): Omit<LoopVerdict, "detector" | "intendedTool"> {
    ...
    if (keys.every((k, i) => k === expected[i])) {
      // Report severity based on event count (pairs * 2) so default thresholds
      // (warning=5, critical=10) actually fire on a 6-event ping-pong.
      return this.severity(6); // 3 pairs = 6 events
    }
