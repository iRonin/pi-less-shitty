...
import type { ExtensionAPI, ResourcesDiscoverEvent, ResourcesDiscoverResult } from "@earendil-works/pi-coding-agent";
...
// HOME resolution must never throw at module load — if it does, the entire
// extension fails to register and pi startup breaks. Fall back to the
// un-realpath'd value when realpathSync fails (e.g. HOME unset, sandboxed
...
function discoverSkills(cwd: string): string[] {
  ...
  // (Removed) hardcoded monorepo fallback for the hindsight skill — it leaked
  // a personal absolute path and was redundant: the cascading walk above
  // already discovers any sibling `packages/hindsight/skills/hindsight`
  // when pi-less-shitty is anywhere under HOME. Drop without replacement.
