...
hindsight_recall(query="build system setup", bank="project-gamma")

// Query multiple banks
hindsight_recall(query="auth patterns", banks=["project-Pi-Agent", "project-gamma", "global"])

// Write directly to a specific bank
hindsight_retain(content="Use jiti for TypeScript extension loading", bank="project-gamma")

// Copy memories between any two banks
hindsight_promote(query="tool preferences", from="project-Pi-Agent", to="global")
```
