...
export function parsePiSession(filePath: string): Session | null {
  try {
    const lines = readFileSync(filePath, "utf-8").trim().split("\n");
    if (lines.length < 3) return null;

    // FIX 4: parse the header defensively. Bare `JSON.parse` on a corrupt
    // first line used to abort the whole session via the outer catch.
    let header: any;
    ...
    // Find model — skip individual corrupt lines instead of bailing the
    // whole session (FIX 4).
    ...
    // Collect user/assistant messages — again skip corrupt lines (FIX 4).
...
// Only auto-run when invoked directly; allow importing the module from tests.
