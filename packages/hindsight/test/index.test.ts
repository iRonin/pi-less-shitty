...
interface AssistantMessage {
  ...
  content?:
    | string
    | Array<{ type: string; name?: string; text?: string; input?: unknown; arguments?: unknown }>;
}
  ...
  for (const msg of messages) {
    ...
    if (typeof content === "string") {
      ...
      for (const block of content) {
        ...
        // Pi uses block.type === "toolCall" with block.arguments. Fall back to
        // "tool_use"/.input for compatibility with Anthropic-shaped fixtures.
        else if ((block.type === "toolCall" || block.type === "tool_use") && !OPERATIONAL_TOOLS.has(block.name)) {
...
describe("buildTranscript", () => {
  ...
  test("includes pi-shape toolCall blocks (regression: previously dropped)", () => {
    // Pi serializes assistant tool calls as { type: "toolCall", arguments: ... },
    // not Anthropic's "tool_use"/.input. The previous implementation only
    // matched "tool_use" and silently dropped every tool call from the retained
    // transcript.
    const messages: AssistantMessage[] = [
      { role: "assistant", content: [{ type: "toolCall", name: "hindsight_retain", arguments: { content: "pi-shape" } }] },
    ];
    const result = buildTranscript("question", messages, []);
    assert.ok(result.includes("[Tool: hindsight_retain]"));
    assert.ok(result.includes('{"content":"pi-shape"}'));
  });

  test("excludes operational tools regardless of block shape (toolCall variant)", () => {
    const messages: AssistantMessage[] = [
...
describe("resolveConfig child-wins", () => {
  test("child bank_id wins over parent bank_id", () => {
    ...
    try {
      ...
      writeHindsightConfig(dir, 'bank_id = "project-main"\nglobal_bank = "project-all"');
      writeHindsightConfig(child, 'bank_id = "project-tools"');

      const result = resolveConfigFixed(child);
      assert.equal(result?.bank_id, "project-tools", "child bank_id should win");
      assert.equal(result?.global_bank, "project-all", "parent global_bank should be inherited");
