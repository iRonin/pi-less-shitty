...
- 28+ extensions loaded as pi packages via `settings.json` → `packages`
- 5 runtime patches managed via the AI-driven `patch-applier` (specs + verify tests survive upstream refactors): `queue-emojis`, `smart-dequeue`, `model-registry-fix`, `agents-listing`, `compaction-tokens`
- Other runtime patches still applied by legacy `session_start` handlers — will be migrated to patch-applier specs
- Upgrades use `scripts/staged-upgrade.sh` which builds pi from `github.com/badlogic/pi-mono` `origin/main`, AI re-applies all patches, smoke-tests, then atomically swaps the global install (auto-rollback on failure)
...
| `@ironin/pi-model-registry-fix` | Fixes apiKey validation (#3043) and models.json wipe (#3044) in model registry |
| `@ironin/pi-name-collision-detector` | Warns when agent and skill names collide (same name in `.pi/agents/` and `.pi/skills/`) |
| `@ironin/pi-patch-applier` | AI-driven runtime patcher — specs + verify + agent-derived edits, replaces brittle string-anchored patchers |
...
|--------|---------|-------------|
| `scripts/staged-upgrade.sh` | Builds pi from `badlogic/pi-mono` `origin/main`, runs patch-applier across all specs, smoke-tests the result, atomically swaps the global install, auto-rolls back on failure | Upgrading pi to a newer upstream — preferred over `npm update` |
| `scripts/rollback.sh` | Restores the most recent backup taken by `staged-upgrade.sh` | Manual rollback if a swap was kept but something broke later |
| `scripts/release-check.ts` | Release verification — packages, patches, agents, skills, context all green | Sanity-check after upgrade or after editing extensions/specs |
| `scripts/apply-all-patches.ts` | Re-runs every legacy dist patcher against the currently installed pi | When a single patch silently broke after `npm update` and you don't want a full staged upgrade |
...
**Migration note:** Migrated to patch-applier specs (preferred path): `queue-emojis`, `smart-dequeue`, `model-registry-fix`, `agents-listing`, `compaction-tokens`. New patches should use patch-applier specs. Legacy `.shitty-pi` files in `skills/dist-patches/patches/` are kept as fallback documentation for unmigrated patches.
...
npx tsx packages/patch-applier/src/cli.ts --check

# Upgrade pi (build from origin/main, AI-patch, atomic swap)
...
For autonomous orchestration, you can create a custom agent to drive the entire upgrade flow.
